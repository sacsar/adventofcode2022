...
auto topThreeCalories(const std::string &input_file) -> int {
  ...
  while (std::getline(ifs, line)) {
    if (!line.empty()) {
      ...
      if (calorie_heap.size() < 3) {
        ...
        if (calorie_heap.size() == 3) {
          std::make_heap(calorie_heap.begin(), calorie_heap.end(),
                         std::greater<>());
        }
      } else {
...
auto maxCalories(const std::string &input_file) -> int {
  std::ifstream ifs(input_file);
